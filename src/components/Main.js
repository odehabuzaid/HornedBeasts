...
class Main extends React.Component {
  ...
  updateFilter = (text) => {
   ...
   //set DataSource FilterByHornsValue
   updateHornValue = (e) => {
     e.preventDefault();
     this.setState({numberOfHorns: e.target.value});
     let filterByHorns = jsonData.slice(0).filter((beast) => {
       if (e.target.value === 'All') { return beast; }
       else { return beast.horns === +e.target.value; }});
     this.setState({beastsData: filterByHorns.slice(0)});
   }
   render() {
     return (
       <main>
                 ...
                 >
                   <Nav.Link href='#'>
                     ...
                     <Form className='d-flex'>
                       ...
                       />
                       <FilterNav className='' updateHornValue={this.updateHornValue} />
                     </Form>
                   </Nav.Link>
                 </Nav>
               </Navbar.Collapse>
